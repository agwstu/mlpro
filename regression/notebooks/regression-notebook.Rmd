...
target_data$year_scaled <- range_to_0_1(target_data$year)
```
...
Two separate summaries are done for timbre average and covariance data. For later modelling purposes two separate DataFrames with target data are specified as well.

```{r}
timbre_average_with_target_data <- timbre_average_data %>%
        add_column(year = target_data %>% pull(year),
                   year_scaled = target_data %>% pull(year_scaled))

timbre_covariance_with_target_data <- timbre_covariance_data %>%
        add_column(year = target_data %>% pull(year),
                   year_scaled = target_data %>% pull(year_scaled))
```
...
Increase in loudness is spotted across decades. Nowadyas people listen more to louder music.
...
```{r}
corr_covariance <-  timbre_covariance_with_target_data %>%
...
t_test_test_across_centuries(timbre_average_with_target_data,
                             'V2')
t_test_test_across_centuries(timbre_average_with_target_data,
                             'V7')
t_test_test_across_centuries(timbre_covariance_with_target_data,
                             'V21')
t_test_test_across_centuries(timbre_covariance_with_target_data,
                             'V15')
```
...
```{r}
timbre_covariance_with_target_for_modelling_data <-
  timbre_covariance_with_target_data %>%
          select(-year) %>%
          slice_head(n = 463715)
```
```{r}
timbre_average_with_target_for_modelling_data <-
    timbre_average_with_target_data %>%
            select(-year) %>%
            slice_tail(n = 51630)
```
...
simple_recipe_with_log <-
        recipe(year_scaled ~ .,
               data = timbre_covariance_with_target_for_modelling_data) %>%
...
cov_tree <- tree(first_model_formula, timbre_covariance_with_target_for_modelling_data)
avg_tree <- tree(first_model_formula, timbre_average_with_target_for_modelling_data)
```
```{r}
summary(cov_tree)
```
...
```{r}
fit_rf_model_with_recipe(simple_recipe_with_log, timbre_covariance_with_target_for_modelling_data)

fit_rf_model_with_recipe(simple_recipe_with_interact, timbre_covariance_with_target_for_modelling_data)
```
