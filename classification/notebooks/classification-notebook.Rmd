...
```{r}
ROC_test_bag <-
  pROC::roc(as.numeric(test_data$class == "good"),
      as.numeric(pred_bag_final))
```
```{r}
list(
  ...
  labs(subtitle = paste0("Gini TEST: ",
                         "bag = ",
                         round(100 * (2 * auc(ROC_test_bag) - 1), 3)
                         ),
       legend = "A") +
...
xgb.ggplot.importance(importance_matrix = importance_matrix,
                      n_clusters = 1) + guides(fill = FALSE) +
...
ROC.test <- pROC::roc(getinfo(xgb_test, "label"), pred_xgb_test)
print(2 * auc(ROC.test) - 1)
```
